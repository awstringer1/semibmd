...
`by` variables in smooths are handled by `mgcv` internally, and the necessary modifications have been
made to `semibmd::benchmark_dose_tmb` to handle them. Here is an example.
...
The `plot` method will plot the smooths for each factor level completely separately. I can change this,
I just did it this way because I didn't have a better idea.
...
Centering and scaling continuous data is common, and is often done internally by regression
software for numerical stability. Centering the data does not affect BMD estimation, but 
scaling does, if not accounted for. Example:

```{r scaling-example-data}
dat_noscale <- dat_shift <- dat_scale <- dat3
dat_shift$y <- with(dat_scale,y-mean(y))
dat_scale$y <- scale(dat_scale$y)

with(dat_noscale,c(mean(y),sd(y)))
with(dat_shift,c(mean(y),sd(y)))
with(dat_scale,c(mean(y),sd(y)))
...
get_bmd(mod_shift)
get_bmd(mod_scale)
get_all_bmdl(mod_noscale)
get_all_bmdl(mod_shift)
get_all_bmdl(mod_scale)
